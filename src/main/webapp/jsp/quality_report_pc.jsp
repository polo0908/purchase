...
<script>
	...
	function btn2(){
		...
		if(key > (li_length-1)){
			...
			$('.quality_report .imgs ul').stop().animate({'left':move1 + 'px'}, 3000);
		}
	...
	function btn1(){
		...
		if(key < 0){
			...
			$('.quality_report .imgs ul').stop().animate({'left':move2 + 'px'}, 3000);
		}
		$('#picPage').text((key+1)+'/'+li_length)
	}
...
if(/(iPhone|iPad|iPod|iOS)/i.test(navigator.userAgent)) {
    window.location.href ="${ctx}/jsp/quality_report.jsp"; // 如果是苹果机，就打开苹果的页面
} else if (/(Android)/i.test(navigator.userAgent)) {//如果是安卓机，就打开安卓页面
    window.location.href ="${ctx}/jsp/quality_report.jsp";
} else {
