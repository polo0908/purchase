...
<div class="customer_complaint_B">	
	<h1 class="customer_complaint_h1">
		质量投诉列表
		<div class="btns">
			<a class="select_blank btn mt5" target="_blank" href="http://112.64.174.34:10010/user/toIndex">返回功能选择页</a>	
			<a href="http://112.64.174.34:10010/jsp/customer_complaint_entry.jsp?roleNo=${roleNo}&userId=${userId}&userName=${userName}" class="btn last">电子质量投诉跟踪单录入</a>				
		</div>
	</h1>
	<div class="form-group scope">
		...
		<select class="form-control display_inline_block" id="solve">
			...
			<option value="2" <c:if test="${solve eq '2'}">selected</c:if>>已解决</option>
		</select>
	</div>
					...
					<ul>
						...
						<li>
						   <c:choose>
							    ...
							    <c:otherwise>
							        ${obj.zhijian1==null?'':obj.zhijian1} ${obj.zhijian2} ${obj.zhijian3}<span class="red"> 未回复</span>
							    </c:otherwise>
							</c:choose>	
						</li>
						<li>
