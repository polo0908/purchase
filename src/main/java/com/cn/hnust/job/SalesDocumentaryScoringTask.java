...
import com.cn.hnust.controller.SalesAndMerchandisingScoreController;
...
@Component
public class SalesDocumentaryScoringTask {
	...
	private final static Logger log = Logger.getLogger(SalesDocumentaryScoringTask.class);
	@Scheduled(cron="0 30 0 * * ?") //每天凌晨30分执行
	public void show(){ 
		log.info("--->SalesDocumentaryScoring go..."+DateFormat.format());
		salesAndDocumentationScoring();
		log.info("--->SalesDocumentaryScoring end..."+DateFormat.format());
	}
