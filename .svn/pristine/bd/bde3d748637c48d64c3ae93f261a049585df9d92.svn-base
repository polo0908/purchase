...
<meta name="Description" content="<c:if test="${projectComplaint.seriousLevel == 1}">一般抱怨</c:if><c:if test="${projectComplaint.seriousLevel == 2}">严重抱怨</c:if><c:if test="${projectComplaint.seriousLevel == 3}">退货</c:if><c:if test="${projectComplaint.seriousLevel == 4}">客户索赔</c:if>_${projectComplaint.customerName}_${projectComplaint.process==null?'':projectComplaint.process}_<c:forEach var="obj" items="${factoryList}" varStatus="i">
				      <c:choose>
...
<body>
<div class="confirm_list confirm_list_share customer_complaint_B_detail">
...
<!-- 	<div class="tc position_absolute text-center alt-message"></div>			 -->
	...
	<input id="project_no" value="${projectComplaint.projectNo}" type="hidden">
	<input id="complaintId" value="${projectComplaint.id}" type="hidden">
		<div class="pull-left top1">
			<h1 class="customer_complaint_h1">电子质量投诉跟踪单</h1>				 
 			<div class="d1"><span>跟踪单编码 — </span><span><fmt:formatNumber value="${projectComplaint.id}" pattern="0000"/></span></div>
			<div class=""><span>投诉单流程：</span>
			<c:if test="${obj.completeTime != null && obj.completeTime != ''}"><span style="color:#08dc2d;">完成</span></c:if>
			<c:if test="${obj.completeTime == null || obj.completeTime == ''}"><span class="red">未完成</span></c:if>
			<c:if test="${projectComplaint.createPerson eq user.userName || user.userName eq 'ninazhao'}">
...
<%-- 		   <c:if test="${type==null || type==''}"> --%>
			...
			<a onclick="goIndex('https://www.kuaizhizao.cn/complaint/queryList')" class="btn select_blank">返回客户投诉列表</a>
...
<%-- 		   </c:if> --%>
			...
			<div class="wrap">
					...
					<div class="wrap3">
						<h4 class="mt10">已归纳【工艺：问题】标签：<span id="s_node" <c:if test="${analysisIssueList==null||analysisIssueList.size()==0}">style="display:none;"</c:if>>暂无</span></h4>
						<ul class="list">
	...
	<c:if test="${user==null||user==''}">
		<div class="share mt10">
			<a onclick="sendReply(11)" style="color:red;background-color: #fff;text-decoration: underline;">您还未登录，如需回复，请先点击登录。</a>
		</div>
	</c:if>
	<div class="share mt10">
	...
	<div class="clearfix mt10">
		<div class="pull-left list_left">
			<p >项目号：<span>${projectComplaint.projectNo}</span></p>
			<p >项目名：<span>${projectComplaint.projectName}</span></p>
			<p >项目阶段：<span>${projectComplaint.projectStage == 0 ? '样品' : '大货'}</span></p>
			<p >客户名：<span>${projectComplaint.customerName}</span></p>
			<p>人员列表：
	...
	<div class="complaint_detail mt10">
		...
		<p><em>严重等级：</em>
		<span>
		<c:if test="${projectComplaint.seriousLevel == 1}">一般抱怨</c:if>
		<c:if test="${projectComplaint.seriousLevel == 2}">严重抱怨</c:if>
		<c:if test="${projectComplaint.seriousLevel == 3}">退货</c:if>
		<c:if test="${projectComplaint.seriousLevel == 4}">客户索赔</c:if>
		</span></p>
	<%-- 	<p><em>项目名：</em><span>${projectComplaint.projectName}</span></p> --%>
		...
		<p>
		${projectComplaint.complaintContent}
		</p>
		<div class="question">		
		<c:forEach var="obj" items="${issueList}" varStatus="i">
			<p class="mt10"><em> 问题${i.index+1}：</em><span>${obj.issue}</span></p>
			      ...
			      <div class="imgs mt10" onclick=" bigPic(this)">			      
			       	<c:choose>
				        <c:when test="${projectComplaint.id > 38}">
				         <img src="http://112.64.174.34:10010/static_img/project_complaint/${projectComplaint.projectNo}/${str}" class="img-responsive" >
				        </c:when>
				        <c:otherwise>
				         <img src="http://112.64.174.34:10010/static_img/project_complaint/${project.productImg}" class="img-responsive">
				        </c:otherwise>
			        </c:choose>
			      </div>
			    </c:forEach>
		...
		<div class="text-right">
			<span>录入人：${projectComplaint.createPerson}</span><span class="ml20"><fmt:formatDate value="${projectComplaint.createTime}" pattern="yyyy-MM-dd"/></span>
		</div>
		<div class="clearfix mt10">
		  <c:if test="${projectComplaint.filePath != null && projectComplaint.filePath != ''}">
			<span class="pull-left">附件名称</span>
			<span>${projectComplaint.fileName}</span>			
			<a href="/complaint/download?id=${projectComplaint.id}" class="btn pull-right">下载</a>
		  </c:if>
		</div>
	</div>
	...
	<div class="depart_reply">
		...
		<div class="btns">
			<div class="btns_1">
	               <c:if test="${user.userName ne 'edward' && user.userName ne 'jerrylong' && user.userName ne 'Jiangwenlong' && user.userName ne 'yanggong' && user.userName ne 'wangweiping'}">	
					   <c:choose>			   
						        <c:when test="${(user.userName eq project.purchaseName && user.userName != '' && user.userName != null) || (user.userName eq project.sellName && user.userName != '' && user.userName != null)}">
							       <a href="/complaint/purchaseReply?id=${projectComplaint.id}"  style="display: inline-block;padding: 0;"><button class="btn" style="background-color: #027CFF; color: rgb(255, 255, 255);">采购/销售录入回复</button></a>
							    </c:when>
						        <c:otherwise>
							       <a onclick="sendReply(1)"  style="display: inline-block;padding: 0;"><button class="btn" style="background-color: #027CFF; color: rgb(255, 255, 255);">采购/销售录入回复</button></a>
							    </c:otherwise>
						</c:choose>	
					</c:if>	
					<c:if test="${user.userName ne 'edward' && user.userName ne 'jerrylong' && user.userName ne 'Jiangwenlong' && user.userName ne 'wangweiping'}">	
						<c:choose>	
							   <c:when test="${(user.userName != '' && user.userName != null) && (user.userName eq project.zhijian1 || user.userName eq project.zhijian2 || user.userName eq project.zhijian3 || user.userName eq 'yanggong')}">
							      <a href="/complaint/inspectionReply?id=${projectComplaint.id}" style="display: inline-block;padding: 0;"><button class="btn" style="background-color: #027CFF; color: rgb(255, 255, 255);">质检录入回复</button></a>
							   </c:when>
							   <c:otherwise>
							      <a onclick="sendReply(2)" style="display: inline-block;padding: 0;"><button class="btn" style="background-color: #027CFF; color: rgb(255, 255, 255);">质检录入回复</button></a>
							   </c:otherwise>
						</c:choose>	
					</c:if>	
					<c:if test="${user.userName ne 'edward' && user.userName ne 'jerrylong' && user.userName ne 'Jiangwenlong' && user.userName ne 'yanggong'}">		
						<c:choose>
						       <c:when test="${user.roleNo == 7 || user.roleNo == 100 || user.roleNo == 99}">
							     <a href="http://112.64.174.34:10010/complaint/technicianReply?id=${projectComplaint.id}" style="display: inline-block;padding: 0;"><button class="btn" style="background-color: #027CFF; color: rgb(255, 255, 255);">技术部录入回复</button></a>
							   </c:when>
							   <c:otherwise>
							     <a onclick="sendReply(3)" style="display: inline-block;padding: 0;"><button class="btn" style="background-color: #027CFF; color: rgb(255, 255, 255);">技术部录入回复</button></a>
							   </c:otherwise>
						</c:choose>		
					</c:if>	
			</div>
			<div class="btns_3">
               <c:choose>
				    <c:when test="${existPurchaseReply == 1}">
					  <button class="btn" onclick="showReply(this)" filed="${existPurchaseReply}" style="background-color: rgb(56, 226, 219); color: rgb(255, 255, 255);">查看采购回复</button>
					</c:when>
					<c:otherwise>
					<button class="btn" filed="${existPurchaseReply}" style="background-color: #e5e5e5; color: rgb(255, 255, 255);">查看采购回复</button>
					</c:otherwise>
				</c:choose>		
				<c:choose>	
				    <c:when test="${existInspectionReply == 1}">
					  <button class="btn" onclick="showReply(this)" filed="${existInspectionReply}"  <c:choose><c:when test="${existPurchaseReply == 0}">style="background-color: rgb(56, 226, 219); color: rgb(255, 255, 255);"</c:when><c:otherwise>style="background-color: #027CFF; color: rgb(255, 255, 255);"</c:otherwise></c:choose>>查看质检回复</button>
					</c:when>
					<c:otherwise>
					<button class="btn" filed="${existInspectionReply}" style="background-color: #e5e5e5; color: rgb(255, 255, 255);">查看质检回复</button>
					</c:otherwise>
				</c:choose>
				<c:choose>
				    <c:when test="${comments!=null || existTechnicianReply == 1}">
					  <button class="btn" onclick="showReply(this)" filed="${existTechnicianReply}" <c:choose><c:when test="${existPurchaseReply == 0 && existInspectionReply == 0}">style="background-color: rgb(56, 226, 219); color: rgb(255, 255, 255);"</c:when> <c:otherwise>style="background-color: #027CFF; color: rgb(255, 255, 255);"</c:otherwise></c:choose>>查看技术部回复</button>
					</c:when>
					<c:otherwise>
					<button class="btn" filed="${existTechnicianReply }"  style="background-color: #e5e5e5; color: rgb(255, 255, 255);">查看技术部回复</button>
					</c:otherwise>
				</c:choose>									
			</div>
<!-- 			<div class="btns_3"> -->
<!-- 				<button class="btn">查看采购回复</button> -->
<!-- 				<button class="btn">查看质检回复</button> -->
<!-- 				<button class="btn">查看技术部回复</button> -->
<!-- 			</div> -->
		</div>
		<div class="replys mt10">
		  
			<div class="reply"  <c:if test="${existPurchaseReply == 0}">style="display:none;"</c:if>>
				<h4><b>采购回复：</b></h4>
				<c:forEach var="obj" items="${issueList}" varStatus="i">
				   <c:forEach begin="1" end="${obj.replyList.size()}" varStatus="j" step="1"> 
				      <c:if test="${obj.replyList.get(j.count-1).replyType == 1}">
				         <p><b>问题${i.index+1}整改方案：</b><span>${obj.replyList.get(j.count-1).replyContent}</span></p>
				      </c:if>
				    </c:forEach>
				</c:forEach>
				<p><b>预计整改完成时间：</b><span><fmt:formatDate value="${projectComplaint.predictCompleteTime}" pattern="yyyy-MM-dd"/></span></p>
				<p><b>预计整改成本：</b><span>${projectComplaint.costAnalysis}</span></p>
				<p><b>根原因分析：</b><span>${projectComplaint.purchaseContent}</span></p>
			</div>
			

			<div class="reply" <c:if test="${existInspectionReply == 0}">style="display:none;"</c:if>>
				<h4><b>质检回复：</b></h4>
				<c:forEach var="obj" items="${issueList}" varStatus="i">
					<c:forEach begin="1" end="${obj.replyList.size()}" varStatus="j" step="1"> 
				      <c:if test="${obj.replyList.get(j.count-1).replyType == 2}">
				       <p><b>问题${i.index+1}：</b><span>${obj.replyList.get(j.count-1).replyContent}</span></p>
				      </c:if>
				    </c:forEach>
				</c:forEach>
				<p><b>其他解释和分析：</b><span>${projectComplaint.inspectionContent}</span></p>				
			</div>
	

			<div class="reply" <c:if test="${comments==null&&existTechnicianReply==0}">style="display:none;"</c:if>>
				<h4><b>技术部回复：</b></h4>
				<c:forEach var="obj" items="${issueList}" varStatus="i">
					<c:forEach begin="1" end="${obj.replyList.size()}" varStatus="j" step="1"> 
				      <c:if test="${obj.replyList.get(j.count-1).replyType == 3}">
					     <a href="/complaint/technicianFile?id=${obj.replyList.get(j.count-1).id}">${obj.replyList.get(j.count-1).fileName}</a><span>点击可下载</span>	
				      </c:if>
				     </c:forEach>
				</c:forEach>	
						...
						<td>								
							<div>
								...
								<c:if test="${obj.newFileName != null && obj.newFileName != ''}">
								   <div>附件：<a href="http://112.64.174.34:10010/static_img/project_complaint/${obj.projectNo}/${obj.newFileName}">${obj.fileName}</a></div>
							    </c:if>
							</div>						 
						</td>	
						</tr>
	...
	<div class="remed_reply mt10">
			<h3><b>整改结果回复：</b></h3>
			<p class="red f18" style="color: red;">任何一个有过问题的项目，在出货前，整改结果都应该 转发到 公司群里面。</p>
			<div class="btns">
			<c:if test="${user.userName ne 'edward' && user.userName ne 'jerrylong' && user.userName ne 'Jiangwenlong' && user.userName ne 'wangweiping'}">	
				<c:choose>
					<c:when test="${existPurchaseReply == 1 && existInspectionReply == 1 }">
					  <a href="/complaint/remediaReply?id=${projectComplaint.id}" style="display: inline-block;padding: 0;"><button class="btn btn-default btn2">录入整改结果</button></a>
					</c:when>
					<c:otherwise>
					  <a style="display: inline-block;padding: 0;"><button class="btn btn-default btn2 e5">录入整改结果</button></a>
					</c:otherwise>
			    </c:choose>
			</c:if>
			</div>
			<div class="reply_detail mt10">
			 <c:if test="${projectComplaint.rectificationSellPurchaseReply != null}">
			 	<p class="clearfix"><em class="pull-left"><b>采购/跟单整改结果录入时间:</b></em><span class="pull-left">
	               <fmt:formatDate value="${projectComplaint.rectificationSellPurchaseTime}" pattern="yyyy-MM-dd"/>
	               </span><span>(${projectComplaint.rectificationSellPurchaseReply})</span></p>
			 </c:if>
			 <c:if test="${projectComplaint.rectificationZhijianReply != null}">
			 	<p class="clearfix"><em class="pull-left"><b>质检整改结果录入时间:</b></em><span class="pull-left">
			 	<fmt:formatDate value="${projectComplaint.rectificationZhijianTime}" pattern="yyyy-MM-dd"/></span>
			 	<span>(${projectComplaint.rectificationZhijianReply})</span></p>
		     </c:if>
			<c:forEach var="obj" items="${issueList}" varStatus="i">
					<c:forEach begin="1" end="${obj.replyList.size()}" varStatus="j" step="1"> 
				      <c:if test="${obj.replyList.get(j.count-1).replyType == 4}">
				<p><em><b>问题${i.index+1}结果：</b></em><span>${obj.replyList.get(j.count-1).replyContent}</span><br/><span><b>质检回复问题${i.index+1}验证结果：</b></span><i>${obj.replyList.get(j.count-1).qualification}</i></p>
                      </c:if>
                   </c:forEach>
              </c:forEach>
			</div>
		...
		<div class="remed_reply mt10">
			<h3><b>投诉单流程：</b> 
			  <c:choose>			    
			     <c:when test="${projectComplaint.completeTime != null}"><em style="color:#08dc2d;">完成</em></c:when>
			     <c:otherwise><em style="color:red;">未完成</em></c:otherwise>
			  </c:choose>
			</h3>			
			<div class="reply_detail mt10">
			<c:choose>
				 <c:when test="${projectComplaint.purchaseConfirmDate != null}">
				 	<p class="clearfix"><em class="pull-left"><b>采购/跟单回复完成日期:</b></em><span class="pull-left blue">
		               <fmt:formatDate value="${projectComplaint.purchaseConfirmDate}" pattern="yyyy-MM-dd"/>
		               </span></p>
				 </c:when>
				 <c:otherwise>
					 <p class="clearfix"><em class="pull-left"><b>采购/跟单回复完成日期:</b></em><span class="pull-left blue">
			 ...
			 <c:choose>
				 <c:when test="${projectComplaint.zhijianReplyTime != null}">
				 	<p class="clearfix"><em class="pull-left"><b>质检回复完成日期:</b></em><span class="pull-left blue">
				 	<fmt:formatDate value="${projectComplaint.zhijianReplyTime}" pattern="yyyy-MM-dd"/></span></p>
			     </c:when>
			     <c:otherwise>
			         <p class="clearfix"><em class="pull-left"><b>质检回复完成日期:</b></em><span class="pull-left blue">
				 	       未回复</span></p>
		     ...
		     <c:choose>
					 <c:when test="${projectComplaint.technicianReplyTime != null || comments != null}">
					 	<p class="clearfix"><em class="pull-left"><b>技术回复完成日期:</b></em><span class="pull-left blue">
					 	<fmt:formatDate value="${projectComplaint.technicianReplyTime}" pattern="yyyy-MM-dd"/></span>
					 	<c:if test="${comments != null}">
					 	   <span class="pull-left blue"><fmt:formatDate value="${comments.get(0).createDate}" pattern="yyyy-MM-dd"/></span>
					 	</c:if>
					 	</p>
				     </c:when>
				     <c:otherwise>
		     ...
		     <c:choose>
				 <c:when test="${projectComplaint.completeTime != null}">
				 	<p class="clearfix"><em class="pull-left"><b>最终整改完成日期:</b></em><span class="pull-left blue">
				 	<fmt:formatDate value="${projectComplaint.completeTime}" pattern="yyyy-MM-dd"/></span></p>
			     </c:when>	
			     <c:otherwise>
			        <p class="clearfix"><em class="pull-left"><b>最终整改完成日期:</b></em><span class="pull-left blue">
				 	        未回复</span></p>
...
<!-- 			 	<p class="clearfix"><em class="pull-left"><b>最终整改质检确认:</b></em>
		...
		<div class="line mt10"></div>
		<div class="mt10 plan">
			<div class="f18"><b>检验计划：</b></div>	
			<table class="table tablered">
				<tbody>		
					<c:forEach var="obj" items="${inspectionPlanList}">		   
							<tr>
								<td>
									<a href="http://112.64.174.34:33168/upload/po/upload/JIANYANJIHUAZJ/${obj.reportName}" class="top">
										<span>${obj.reportName}</span>
										<span><fmt:formatDate value="${obj.createDate}" pattern="yyyy-MM-dd"/></span>
									</a>
								</td>
								<td><a href="http://112.64.174.34:33168/upload/po/upload/JIANYANJIHUAZJ/${obj.reportName}"  class="btn btn-default">查看</a></td>
							</tr>
					</c:forEach>							
				</tbody>
			</table>		
		...
		<div class="line mt10"></div>
	<div class="list mt10">
		<div class="f18"><b>本项目质检报告列表：</b></div>		
			<table class="table tablered">
				<tbody>
				   <c:forEach var="report" items="${qualityReports}">
						<tr>
							...
							<td><a onclick="window.location='https://www.kuaizhizao.cn/quality/shareQuality?id=${report.id}'" class="btn btn-default">查看</a></td>
						</tr>
					</c:forEach>
					<c:if test="${qualityReports == null || qualityReports.size() == 0}">
					   暂无
					</c:if>
				</tbody>
			</table>				
  ...
  <div class="list task mt10">
				  ...
				  <c:forEach var="obj" items="${tasks}" varStatus="i">		
					<tr>
						<td>
							<p>${obj.description}</p>
							<div>
						        <span>${obj.initiator}</span>
								<span>${obj.accepter}</span>
								<span><fmt:formatDate value="${obj.startTime}" pattern="yyyy/MM/dd"/></span>
							</div>
					   </td>
						<td>
					     <c:if test="${obj.taskStatus == 0}"><span class="red">未完成</span></c:if>
							 <c:if test="${obj.taskStatus == 1}"><span>已完成</span></c:if>
							 <c:if test="${obj.taskStatus == 2}"><span>已暂停</span></c:if>
							 <c:if test="${obj.taskStatus == 3}"><span>已取消</span></c:if>
							 <a href="http://112.64.174.34:10010/projectTask/selectProjectTaskById?id=${obj.id}" class="display_block">查看</a>
						</td>
					</tr>
					</c:forEach>
	...
	<div class="mt10">		
		<div>
			...
			<c:choose>
				<c:when test="${analysisIssueList != null && analysisIssueList.size() > 0}">
				   <span class="blue">	
				       <c:forEach var="obj" items="${analysisIssueList}" varStatus="i">	
	...
	<div class="share mt10" style="text-align: center;">
	   <c:if test="${fn:containsIgnoreCase(user.userName, projectComplaint.createPerson) || user.userName eq 'ninazhao'}">
...
$(function(){
     ...
     a = a + "&type=1";
     img = productImg?'http://112.64.174.34:10010/product_img/'+productImg:'';
